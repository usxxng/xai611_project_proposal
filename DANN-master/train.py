...
dataset_sizes_trg = {}
dataset_sizes_trg["train"] = int(0.8 * len(target_dataset))
dataset_sizes_trg["val"] = len(target_dataset) - dataset_sizes_trg["train"]


datasets = {}
datasets_trg = {}
datasets["train"], datasets["val"] = torch.utils.data.random_split(source_dataset, [dataset_sizes["train"], dataset_sizes["val"]])
datasets_trg["train"], datasets_trg["val"] = torch.utils.data.random_split(target_dataset, [dataset_sizes_trg["train"], dataset_sizes_trg["val"]])
...
train_loader_target = torch.utils.data.DataLoader(datasets_trg["train"], batch_size=args.batch_size, shuffle=True)
valid_loader_target = torch.utils.data.DataLoader(datasets_trg["val"], batch_size=args.batch_size, shuffle=False)
